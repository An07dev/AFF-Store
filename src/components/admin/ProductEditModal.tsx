...
import { FiX, FiPlus, FiTrash2, FiUploadCloud, FiCheck, FiEdit3 } from 'react-icons/fi';
...
export default function ProductEditModal({
          ...
          setVariants(
            prod.variants && prod.variants.length > 0
              ? prod.variants.map((v: any) => ({
                  ...
                  size: v.size || '',
                  stock: v.stock ?? 0,
                  price: v.price ?? (prod.salePrice || prod.price),
                }))
              : []
          );
        } else {
  ...
  const handleAddVariant = () => {
    setVariants((prev) => [
      ...prev,
      {
        ...
        price: parseFromDots(formData.salePrice || formData.price) || 0,
      },
    ]);
  };
  ...
  // Submit Form using API 2.4 PUT specs
